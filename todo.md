...
> Memory — overview + COW + LTM + schema; Async — execution + streaming; RE-PLAN — overview + checkers/aggregation; Evaluation — metrics + datasets + reflection; Evolution — evolver + mutation/results; Tracing — observability + viz + cost; LLM-clients — clients + retries + record/replay; Orchestration — supervisor/handoff/debate/sampling/human-in-loop + events; AgentSkills — overview + resolvers + sandbox; MCP — tools + resources; Serialization — JSON + migration; Cookbook — catalog + end-to-end; Reference — API + changelog.
> Все примеры сверены с исходником 0.3.0 (точные поля/импорты).
> (сейчас Starlight отдаёт EN как фолбэк). **✅ CARL EN полностью готов (§3.1–3.19).**
> Дальше по плану: CARE (Фаза 2), RU-зеркало CARL, полировка (Фаза 4).
