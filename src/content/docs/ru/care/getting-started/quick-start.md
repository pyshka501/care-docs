---
title: Быстрый старт
description: Запустите MAESTRO CARE и сгенерируйте первую цепочку в формате CARL за пять минут.
sidebar:
  order: 1
---

От чистого клона репозитория до работающего агента — примерно за пять минут.
...
запишет `./.env`, чтобы свежий клон сразу запускался:
...
MAESTRO CARE открывается сразу в **чат-поверхности** — транскрипт в стиле
...
MAGE сгенерирует цепочку, MAESTRO CARE тут же её выполнит, и ответ выведется
