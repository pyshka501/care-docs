...
When you type a task in Production mode, CARE runs this sequence automatically:
...
4. **Baseline** — CARE runs **one baseline execution** and persists it as the
   **first dataset entry** for that chain.
5. **Evolve** — if a [Platform](/care/concepts/architecture/) is wired and the
   baseline succeeded, CARE **kicks off an evolution run** against the baseline.
